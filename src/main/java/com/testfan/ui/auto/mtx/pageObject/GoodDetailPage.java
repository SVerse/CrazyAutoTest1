...
private static Logger log = Logger.getLogger(GoodDetailPage.class);//用于eclipse工程内运行查找对象库文件路径
 public   GoodDetailPage(DriverBase driver) {
//工程内读取对象库文件
	super(driver);
...
* @throws IOException
*/
public  ElementBeans buyNow() throws IOException
 {
   if(locatorMap.containsKey("buyNow")||locatorMap.containsKey("ios_buyNow")){
   ElementBeans elementBeans=locatorMap.get("buyNow");
       if(driver.platform.equalsIgnoreCase("ios")){
           elementBeans=locatorMap.get("ios_buyNow");
       }
   return elementBeans;
 }
   else{
 log.error("在"+pagePath+"中不存在【buyNow】元素信息"); return null;}
 }
